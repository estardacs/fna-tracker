...
//
// Upstream incluye "--no-sandbox" y "--disable-setuid-sandbox" en esta lista fija. Esos
// dos flags apagan el sandbox de Chrome, que es justamente la barrera que impide que una
// pestaña comprometida toque el resto de la máquina. Apagarlo mientras se navega el portal
// de un banco es al revés de lo que queremos.
//
// Solo existen porque en contenedores corriendo como root Chrome no arranca sin ellos. En
// un escritorio Windows o macOS no hacen falta, así que acá son opt-in explícito: si algún
// día este colector se empaqueta a Cloud Run, se setea OBC_ALLOW_NO_SANDBOX=1 y se acepta
// el costo a conciencia, en vez de heredarlo sin darse cuenta.
const SANDBOX_OFF_ARGS = ["--no-sandbox", "--disable-setuid-sandbox"];
...
const DEFAULT_ARGS = [
  ...
  ...(process.env.OBC_ALLOW_NO_SANDBOX === "1" ? SANDBOX_OFF_ARGS : []),
];
