...
function Bar({ label, consumed, goal, unit = 'g', colorClass }: MacroBarProps) {
...
export default function MacroBars({ consumed, goal }: MacroBarsProps) {
  return (
    <div className="space-y-3 flex-1 min-w-[200px]">
      ...
      <Bar label="Proteína" consumed={consumed.protein_g} goal={goal.protein_g} colorClass="bg-blue-500" />
      <Bar label="Carbohidratos" consumed={consumed.carbs_g} goal={goal.carbs_g} colorClass="bg-amber-500" />
      <Bar label="Grasas" consumed={consumed.fat_g} goal={goal.fat_g} colorClass="bg-pink-500" />
      <Bar label="Fibra" consumed={consumed.fiber_g} goal={0} colorClass="bg-green-500" />
    </div>
  );
}
